<template>
  <div class="product-cards">
    <ProductPaginator
      @goTo="getPage"
      :count="products ? parseInt(products.count) : searchTerms.limit"
      :currentPage="currentPage"
      :limit="searchTerms.limit"
    />
    <div
      class="product-cards__display"
      :class="{'product-cards__display--empty': !productsLoading && products.count === '0'}"
    >
      <div
...
export default {
  ...
  computed: {
    ...
    pageKey() {
      let key = "";
      key +=
        this.searchTerms.department_id &&
        `Department:${this.searchTerms.department_id}`;
      key +=
        this.searchTerms.category_id &&
        `Category:${this.searchTerms.category_id}`;
      key += `Page:${this.currentPage}`;
      // Key must be unique in order to ensure the transition works whenever the page changes
      return key;
    },
    filteredProducts() {
      const products = this.products.product_list;
      if (this.searchTerms.department_id && this.searchTerms.category_id) {
        return products.filter(product => {
          return (
            parseInt(product.locations.department.department_id) ===
              this.searchTerms.department_id &&
            parseInt(product.locations.category.category_id) ===
              this.searchTerms.category_id
          );
        });
      } else {
