...
export default {
  login: async (parent, {
    ...
    try {
      ...
      if (!userExists) {
        ...
        } = await axios.get(`${endpoint}/shoppingcart/generateUniqueId`)
  ...
  createOrder: async (parent, {
    ...
    try {
      ...
      const {
        data
      } = await axios.post(`${endpoint}/stripe/charge`, {
  ...
  getNewCart: async (parent, {
    ...
    await prisma.mutation.updateCustomer({
      where: {
        turing_id: parseInt(customerId)
      },
      data: {
        cart: {
          connect: {
